...
async function getIndexers(host, apiKey) {
  ...
  const response = await axios.get(`${host}/api/v2.0/indexers/all/results/torznab/api`, {
    params: params,
...
async function getIndexerTorrentMap(info, indexers, host, apiKey) {
  ...
  return await Promise.all(indexers.map(async (indexer) => {
    ...
    const response = await axios.get(`${host}/api/v2.0/indexers/${indexer.attributes.id}/results/torznab/api`, {
      params: params,
      timeout: config.requestTimeoutMs,
      responseType: 'text',
    })
...
function parseIndexerTorrents(indexer, torrents, host, type, id) {
  ...
  elements.forEach(element => {
    ...
    element.elements.forEach(subElement => {
      if (subElement.name == 'torznab:attr' && subElement.attributes && subElement.attributes.name && subElement.attributes.value)
...
async function parseTorrent(torrent) {
  ...
  if (!response || response.status != 200 || !response.data) {
    ...
    config.debug && console.error("Error fetching torrent data: ", response.statusText)
    return null
  }
...
async function search(info, host, apiKey) {
  ...
  const indexerTorrents = indexerTorrentMap.flatMap(indexerTorrentMap => parseIndexerTorrents(indexerTorrentMap[0], indexerTorrentMap[1], host, info.type, info.id))
  config.debug && console.log("indexerTorrents:", JSON.stringify(indexerTorrents))

  const torrents = await Promise.all(indexerTorrents.map(async indexerTorrent => {
